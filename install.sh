...
while getopts "d:p:" opt; do
  case $opt in
    d)
      export REMOTELABZ_WORKER_PATH="$OPTARG"
      ;;
    p)
...
if [ -z "$REMOTELABZ_WORKER_PATH" ]; then
    export REMOTELABZ_WORKER_PATH=/var/www/html/remotelabz-worker
fi
...
if ! [ $(grep -Fxq "www-data     ALL=(ALL) NOPASSWD: /bin/ip" /etc/sudoers.d/www-data) ]; then
    echo "www-data     ALL=(ALL) NOPASSWD: /bin/ip" | tee /etc/sudoers.d/www-data >> "${INSTALL_LOG}" 2>&1
fi
# Composer
