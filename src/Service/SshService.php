...
class SshService
{
    ...
    /**
     * Receives a file from a remote server via SCP (reverse copy).
     * @param resource $connection SSH connection
     * @param string $remoteFile Path to file on remote server
     * @param string $localFile Path where to save file locally
     * @return bool True on success, false on failure
     */
    public function receiveFile($connection, $remoteFile, $localFile): bool
    {
        $this->logger->debug(
            "[SshService:receiveFile]::Receiving file " . $remoteFile . " -> " . $localFile
        );
        
        try {
            $success = ssh2_scp_recv($connection, $remoteFile, $localFile);
            
            if (!$success) {
                throw new ErrorException('Failed to receive file');
            }
            
            $this->logger->info("File received successfully via scp: " . $localFile);
            return true;
            
        } catch (ErrorException $e) {
            $this->logger->error(
                "Failed to receive file " . $remoteFile . " -> " . $localFile
            );
            $this->logger->error($e->getMessage());
            return false;
        }
